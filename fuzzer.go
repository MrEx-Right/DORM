...
func NewFuzzer(targetURL string) *Fuzzer {
	...
	payloads := loadPayloads("wordlists/fuzzing.txt")

	// Fallback if file is missing (Safety Net)
	if len(payloads) == 0 {
		fmt.Println("[!] Warning: wordlists/fuzzing.txt not found! Using default list.")
...
func (p *FuzzerPlugin) Run(target models.ScanTarget) *models.Vulnerability {
