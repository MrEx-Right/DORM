...
This major release introduces the **Supply Chain Interface (SCI)** as the centerpiece of DORM, providing deep technology fingerprinting and version-aware vulnerability matching. It also brings critical security patches to the frontend and fine-tunes severity classifications.

---

#### 🧩 Supply Chain Interface (SCI) Added
- **Deep Supply Chain Analysis:** Introduced a powerful new interface accessible via the left sidebar. SCI deeply analyzes a target to detect all components—from frontend libraries to backend infrastructure, CMS, and CDNs.
- **Version-Aware CVE Enrichment:** The SCI engine actively hunts for component versions and cross-references them with the local CVE database. Components with unknown versions are marked as `SAFE` to avoid false positives, ensuring only highly accurate vulnerabilities are reported.
- **Standalone Execution:** Built as an independent, on-demand analysis tool rather than a passive plugin. You can trigger an SCI scan at any time without starting a full target scan.
- **Modular Signatures via JSON:** Powered by a highly scalable engine that dynamically loads over 80+ component detection signatures from independent JSON files (`sci/signature/`). This makes expanding the detection capabilities effortless.

#### 🛡️ Frontend Security Patches
- **DOM-Based XSS Remediation:** Completely refactored the `escapeHtml` function in the frontend (`app.js`) to use a secure, Regex-based encoding mechanism that properly sanitizes both single and double quotes.
- **Secure Inline Handlers:** Neutralized severe JavaScript string injection vectors by eliminating raw string concatenations inside inline event handlers (e.g., `onclick="deleteScan('${rec.id}')"`). These now utilize HTML5 `data-id` attributes and `dataset` properties to ensure data is safely parsed by the DOM.
- **Form Type Sanitization:** Fixed an issue in the Sitemap view where dynamically generated form types (e.g., `type-${inp.type}`) were directly injected into the DOM as class names without any sanitization.

#### 📊 Severity Reclassification
- **Informational Findings Adjusted:** Re-categorized purely informational findings generated by various plugins (such as Service Banner, HSTS Missing, Missing Security Headers, Insecure Cookie, Clickjacking, Swagger docs, S3 Bucket Links, Apache Status Page, and .DS_Store file). These were previously flagged as `LOW` severity with artificial CVSS scores (2.0 - 4.0), and are now accurately classified as `INFO` severity with a CVSS score of `0.0`.
